...
class TrainingThreadGRU(QThread):
    ...
    def run(self):
        ...
        if self.fast_mode:
            ...
            epochs = 8
            batch_size = 32
            self.logModel.update_estimated_time("00:50")
        else:
            n_iter = 10
            epochs = 40
            batch_size = 64
        ...
        search = RandomizedSearchCV(model, param_distributions, n_iter=n_iter, cv=3, scoring=self.custom_scorer, verbose=1)
        ...
        relatorio.save_roc_pr_curves_regression(self.categories_test, test_results, "Modelo GRU", self.category_thresholds)
    ...
    def save_model(self,model):
        ...
        print(f"Modelo GRU salvo em: {h5_filename}")
