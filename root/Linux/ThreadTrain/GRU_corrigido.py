#!/usr/bin/env python3
"""
GRU Corrigido - Versão sem vazamento temporal e com balanceamento de classes
Correções aplicadas:
...
from tensorflow.keras.layers import GRU, Dense, Dropout, GaussianNoise, BatchNormalization # type: ignore
...
class KerasGRURegressorBalanced(BaseEstimator, RegressorMixin):
  ...
  def _build_model(self):
    self.model = Sequential([
      ...
      # GRU principal com dropout interno e recorrente
      GRU(self.units, activation=self.activation,
      ...
      # Segunda camada GRU para maior capacidade
      GRU(self.units//2, activation=self.activation,
        dropout=self.dropout, recurrent_dropout=self.dropout,
...
class TrainingThreadGRUCorrigido(QThread):
  ...
  def build_data_corrected(self, data, balancing_technique='class_weights'):
    ...
    # 6. Preparar para GRU (adicionar dimensão temporal)
  ...
  def run(self):
    ...
    model = KerasGRURegressorBalanced(
...
def main():
  """Teste da versão GRU corrigida"""
  print("Testando GRU Corrigido com Ruído")
  print("="*50)
