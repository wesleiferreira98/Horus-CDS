...
plt.scatter(pd.to_datetime(X_test['TXTDATE']), y_test, label='Valor Real', color='blue')

# Plotar as previsões
plt.scatter(pd.to_datetime(X_test['TXTDATE']), y_pred, label='Previsão', color='green')

# Plotar as previsões para os próximos 10 dias
plt.scatter(previsoes_10_dias['TXTDATE'], previsoes_10_dias['Previsao_LONGTIME'], label='Previsão para 10 dias', color='orange')
